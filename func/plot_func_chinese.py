...
def bler_chart(accs, args):    
    ...
    for plot in plots:
        # plt.plot(range(0, len(accs[plot])), accs[plot], color = colors_map[plot], marker=markers_map[plot], markerfacecolor='none', label=str(plot).capitalize() + " Acc")
        # plt.plot(range(0, len(accs[plot])), accs[plot], color = colors_map[plot], label=str(plot).capitalize() + " Acc")
        plt.plot([0, 1]+[args.plot_interval * i for i in list(range(1, len(accs[plot])-1))], [1-x for x in accs[plot]], color = colors_map[plot], linestyle = line_map[plot], label=str(plot).capitalize())
...
def ber_chart(ber, args):    
    ...
    for plot in plots:
        # plt.plot(range(0, len(accs[plot])), accs[plot], color = colors_map[plot], marker=markers_map[plot], markerfacecolor='none', label=str(plot).capitalize() + " Acc")
        # plt.plot(range(0, len(accs[plot])), accs[plot], color = colors_map[plot], label=str(plot).capitalize() + " Acc")
        plt.plot([0, 1]+[args.plot_interval * i for i in list(range(1, len(ber[plot])-1))], ber[plot], color = colors_map[plot], linestyle = line_map[plot], label=str(plot).capitalize())
...
def plot_embedding_2D(data, label, epoch,len_covert):
    ...
    plt.scatter(data[0:len_covert,0], data[0:len_covert,1], c="none", marker="o", edgecolors='red', s=12, label="数据")
    plt.scatter(data[len_covert:,0], data[len_covert:,1], c="none", marker="^", edgecolors='gray',s=12, label="噪声")
    plt.xlim(0, 1)
    plt.ylim(0, 1)
    ...
    plt.xlabel("维度0", fontsize=10)
    plt.ylabel("维度1", fontsize=10)
...
def constellation_image(sample_data, real_data, args, epoch):
    ...
    plt.scatter(sample_data[:,0], sample_data[:,1], c="red", marker="+", s=10, label="数据")
    plt.scatter(real_data[:,0], real_data[:,1], c="gray", marker=".", s=10, label="噪声")
