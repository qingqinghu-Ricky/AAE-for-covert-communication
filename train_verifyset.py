...
def train(args, models, data_loader, verify_ds, lambdas=(1, 1), optim_fn=torch.optim.Adam):
    ...
    losses = {'alice_tra': [], 'willie_tra': [], 'bob_tra': []}
    accs = {'alice_val':[], 'willie_val': [], 'bob_val': []}
    PDF_diff = {'JS_val': []}
    ber = {'bob_val': []}
    ...
    with torch.no_grad():
        ...
        bob_acc0 = bob_accuracy(bob_decode0, verify_y) # 计算Bob按block判决的准确度
    ...
    for epoch in range(args.total_epoch):
        ...
        # snr = torch.randint(10, 21, (1,), requires_grad=False).to(device) # 产生[10,20]的均匀分布的随机整数
        ...
        for i, (x, y) in enumerate(data_loader):
            ...
            # noise_data = torch.randn(args.batch_size, args.n_channel).to(device)
            t = torch.randn(args.batch_size, args.n_channel).to(device) # n长度的随机输入向量t
            _, _, noise_data = ae(x, t, snr) # 默认调用forward函数, 每个batch固定一个噪声样本
            ...
            for i_ in range(w_num):
                ...
                # 训练 willie网络，能检测是 noise信号还是 covert信号——————————————————————————————————————————————————
                t = torch.randn(args.batch_size, args.n_channel).to(device) # n长度的随机输入向量t
                bob_decode, covert_aftchannel, _ = ae(x, t, snr) # 默认调用forward函数

                
                willie_noise_output = w(noise_data.detach())
                willie_covert_output = w(covert_aftchannel.detach()) # 生成器部分不参与梯度

                # willie_loss = w_criterion(willie_covert_output, torch.zeros((willie_covert_output.shape[0],1)).to(device)) + w_criterion(willie_noise_output, torch.ones((willie_noise_output.shape[0],1)).to(device))
                willie_loss = torch.mean(willie_covert_output) - torch.mean(willie_noise_output) + 10*gradient_penalty(noise_data, covert_aftchannel, w, device)
                if np.isnan(willie_loss.detach().numpy()) == False:   # 判断输出是否有NaN
            ...
            for i_ in range(a_num):
                ...
                willie_output = w(covert_aftchannel)
                Willie_loss1 = -torch.mean(willie_output)
                bob_loss1 = ae_criterion(bob_decode, y)
                
                    
                alice_loss = willie_lambda * Willie_loss1+ bob_lambda * bob_loss1 # 同时考虑 Willie和Bob
                # alice_loss = -torch.mean(willie_output) # 只考虑willie 
                if np.isnan(alice_loss.detach().numpy()) == False:   # 判断输出是否有NaN
                    alice_loss.backward()
                    ae_encoder_optimizer.step()
                    # ae_sigma_optimizer.step()
                else:
                    import pdb; pdb.set_trace()
            ...
            train_willie_loss.append(willie_loss.item()) # item 返回标量数字
            train_alice_loss.append(alice_loss.item())
        ...
        if (epoch+1) % args.plot_interval == 0 or epoch == 0:
            ...
            print('Train Epoch[{}/{}], Loss_B:{:.4f}, Acc_B:{:.3f}, Loss_A:{:.4f}, Loss_W:{:.4f}, Acc_A:{:.3f}, Acc_W:{:.3f},  JS:{:.3e}, ber: {:.3e}, SNR:{}'.format(
            epoch + 1, args.total_epoch,  np.mean(train_bob_loss), bob_acc_verify, np.mean(train_alice_loss), np.mean(train_willie_loss), alice_acc_verify, willie_acc_verify, JS_verify, bob_ber_verify, snr.numpy()))
...
if __name__ == '__main__': # 默认整个脚本从此处开始运行
    def parse_args():
        ...
        parser.add_argument('--plot_interval', type=int, default = 3000 ) # 绘图epoch间隔 50, 100
